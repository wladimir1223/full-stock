...
// Valida y sanitiza el payload según el esquema de la colección
...
function listItems(req, res) {
  ...
  const items = db.getItems(req.tenant.id, req.params.slug);
...
function getItem(req, res) {
  const schema = db.getSchema(req.tenant.id, req.params.slug);
  if (!schema) return res.status(404).json({ success: false, message: 'Colección no encontrada.' });
  const item = db.getItem(req.tenant.id, req.params.slug, req.params.id);
...
function createItem(req, res) {
  ...
  db.createItem(req.tenant.id, req.params.slug, item);
...
function updateItem(req, res) {
  ...
  const existing = db.getItem(req.tenant.id, req.params.slug, req.params.id);
  if (!existing) return res.status(404).json({ success: false, message: 'Item no encontrado.' });

  const { errors, data } = validateAndSanitize(schema.fields, req.body);
  if (errors.length > 0) return res.status(400).json({ success: false, errors });

  const updated = db.updateItem(req.tenant.id, req.params.slug, req.params.id, {
    ...data,
...
function deleteItem(req, res) {
  ...
  const deleted = db.deleteItem(req.tenant.id, req.params.slug, req.params.id);
