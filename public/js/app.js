...
const App = (() => {
  ...
  // ─── Grupos colapsables (la key del grupo se lista en NAV_SECTIONS) ──────────
  const NAV_GROUPS = {
    monitor: {
      label:    'Monitoreo Global',
      iconKey:  'monitor',
      children: ['monitor_logs', 'monitor_users'],
    },
  };
  ...
  function getPanels() {
    ...
    if (isSuperAdmin()) {
      base.monitor_logs  = { label: 'Registro de Actividades', module: () => SuperAdmin, subview: 'logs',  parentGroup: 'monitor' };
      base.monitor_users = { label: 'Usuarios',                module: () => SuperAdmin, subview: 'users', parentGroup: 'monitor' };
    ...
    return NAV_SECTIONS
      .map(sec => {
        const items = sec.keys
          .map(k => {
            if (NAV_GROUPS[k]) {
              const grp      = NAV_GROUPS[k];
              const children = grp.children.filter(c => PANELS[c]);
              return children.length ? renderNavGroup(k, grp, children) : null;
            }
            return PANELS[k] ? renderNavItem(k) : null;
          })
          .filter(Boolean);
  ...
  function renderNavGroup(groupKey, grp, children) {
    const childButtons = children.map(c => `
      <button class="nav-tab w-full flex items-center gap-2.5 pl-8 pr-3 py-1.5
                     rounded-lg text-[0.7875rem] font-medium text-slate-500
                     hover:bg-white/5 hover:text-slate-100 transition-all duration-150"
              data-panel="${c}">
        ${NAV_ICONS[c] || ''}
        ${PANELS[c].label}
      </button>`).join('');
    ...
    return `
      <div>
        <button class="nav-group-btn w-full flex items-center gap-2.5 px-3 py-2
                       rounded-lg text-[0.8125rem] font-medium text-slate-400
                       hover:bg-white/5 hover:text-white transition-all duration-150"
                data-group="${groupKey}">
          ${NAV_ICONS[grp.iconKey] || ''}
          <span class="flex-1 text-left">${grp.label}</span>
          <svg data-group-arrow="${groupKey}"
               xmlns="http://www.w3.org/2000/svg"
               class="w-3.5 h-3.5 shrink-0 text-slate-600"
               fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
          </svg>
        </button>
        <div class="nav-group-children" data-group-children="${groupKey}">
      ...
      <div class="px-3 py-3 border-t border-white/5 shrink-0">
        <a href="/tienda/${escHtml(slug)}" target="_blank" rel="noopener"
           class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium
                  text-slate-500 hover:text-indigo-300 hover:bg-white/5 transition-all duration-150">
  ...
  function renderApp() {
    ...
    document.body.innerHTML = `
      ...
      <div class="h-screen bg-[#0B0F19] text-slate-100 flex flex-col overflow-hidden">

        <!-- ══ Header ══════════════════════════════════════════════════════════ -->
        <header class="h-14 bg-slate-900/50 backdrop-blur-md border-b border-white/5 px-4
        ...
        <div class="flex flex-1 overflow-hidden">
          ...
          <aside class="hidden lg:flex flex-col w-56 bg-slate-900/50 backdrop-blur-md border-r border-white/5 shrink-0">
          ...
          <aside id="nav-drawer"
                 class="nav-drawer-panel fixed top-0 left-0 h-full w-64 bg-slate-900/80 backdrop-blur-xl
                        border-r border-white/5 z-50 flex flex-col lg:hidden">
            <!-- Cabecera del drawer -->
        ...
        <!-- ══ Footer ══════════════════════════════════════════════════════════ -->
        <footer class="h-9 bg-slate-900/50 backdrop-blur-md border-t border-white/5 px-5
  ...
  function bindAppEvents() {
    ...
    // Nav group toggles — submenús colapsables (CSP-safe)
    document.querySelectorAll('.nav-group-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const key    = btn.dataset.group;
        const isOpen = !!document.querySelector(`[data-group-children="${key}"]`)
                                 ?.classList.contains('is-open');
        document.querySelectorAll(`[data-group-children="${key}"]`)
          .forEach(el => el.classList.toggle('is-open', !isOpen));
        document.querySelectorAll(`[data-group-arrow="${key}"]`)
          .forEach(el => el.classList.toggle('is-open', !isOpen));
      });
    });
  ...
  async function navigate(panelKey, opts) {
    ...
    const panelDef = PANELS[panelKey];
    ...
    // Auto-expandir grupo padre si el panel pertenece a uno
    if (panelDef.parentGroup) {
      const gKey = panelDef.parentGroup;
      document.querySelectorAll(`[data-group-children="${gKey}"]`)
        .forEach(el => el.classList.add('is-open'));
      document.querySelectorAll(`[data-group-arrow="${gKey}"]`)
        .forEach(el => el.classList.add('is-open'));
    }
    ...
    try {
      const mergedOpts = Object.assign({ subview: panelDef.subview }, opts || {});
      await PANELS[panelKey].module().render(panelDiv, mergedOpts);
    } catch (err) {
      panelDiv.innerHTML =
